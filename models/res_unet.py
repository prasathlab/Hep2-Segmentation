...
def stem(x, filters, kernel_size=(3, 3), padding="same", strides=1):
    ...
    conv = conv_block(x, filters, kernel_size=kernel_size,
                      padding=padding, strides=strides)

    shortcut = keras.layers.Conv2D(filters, kernel_size=(
...
def get_res_unet_model():
    ...
    outputs = keras.layers.Conv2D(
        1, (1, 1), padding="same", activation="sigmoid")(d4)
    model = keras.models.Model(inputs, outputs)
    return model


def get_model_with_loaded_weights():
    model_with_all_weights = get_res_unet_model()
    model_with_all_weights.load_weights("Hep2-Segmentation/models/ResUNet.h5")
